...
function Exchange(props) {
  ...
  const quateSwap = async (e) => {
    ...
    try {
      ...
      var amountsOut , outputAmount
      if (fromAddress.address === WETH) {
        amountsOut = await contract.methods
          .getAmountsOut(`${amountIn}`, [WETH, toAddress.address])
          .call();

        outputAmount = amountsOut[1] / 10 ** toAddressDecimals;
        ...
        amountsOut = await contract.methods
          ...
          .call();
        outputAmount = amountsOut[2] / 10 ** toAddressDecimals;
      }
  ...
  const swap = async (e) => {
    ...
    amountIn = parseFloat(fromAmount) * pow;
    var amountsOut, amountOutMin
    if (fromAddress.address === WETH) {
      amountsOut = await contract.methods
        .getAmountsOut(`${amountIn}`, [WETH, toAddress.address])
        .call();
      ...
      if (fromAddress.address === WETH) {
        tx = await contract.methods
          .swapExactETHForTokens(
            ...
            account,
            Date.now() + 1000 * 60 * parseInt(deadLine) 
          )
  ...
  const getTokenBalance = async (tokenAddress, type) => {
    try {
      if (type === "From") {
        var result, tokenContract, format, decimals, getBalance, pow, balanceInEth
        if (tokenAddress === "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c") {
          result = await web3Obj.eth.getBalance(account);
          format = web3Obj.utils.fromWei(result); 
          setFromBalance(parseFloat(format).toFixed(10));
        } else {
          tokenContract = new web3Obj.eth.Contract(WBNB, tokenAddress);
          decimals = await tokenContract.methods.decimals().call();
          getBalance = await tokenContract.methods
            .balanceOf(account)
            .call();

          pow = 10 ** decimals;
          balanceInEth = getBalance / pow;
